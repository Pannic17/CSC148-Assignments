        ...
        if self.is_leaf():
            ...
            if compare_list(self.value, prefix):
                ...
                for index in list_index:
                    output.extend(self.subtrees[index].
                                  autocomplete_helper(prefix))
        return output
...
def sort_trees(list_trees: List) -> List[int]:
    """
    sort the trees and return a list of index for autocomplete function
    """
    index = 0
    list_tup = []
...
class CompressedPrefixTree(Autocompleter):
    ...
    def insert_delete(self, prefix: List) -> None:
        """
        Helper Function / delete the unnecessary node for insert
        /Recursion
        """
        if self.is_leaf():
            pass
