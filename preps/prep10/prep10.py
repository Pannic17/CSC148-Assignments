...
class BoolOp(Expr):
    ...
    def evaluate(self) -> Any:
        ...
        if self.op == 'and':
            output = True
            for judgement in self.values:
                output *= judgement.evaluate()
            return bool(output)
        elif self.op == 'or':
            output = False
            for judgement in self.values:
...
class Compare(Expr):
    ...
    def evaluate(self) -> Any:
        ...
        """
        judgement = []
        num = self.left.evaluate()
        for tup in self.comparisons:
            if tup[0] == '<=':
                if num <= tup[1].evaluate():
                    judgement.append(Bool(True))
                else:
                    judgement.append(Bool(False))
            elif tup[0] == '<':
                if num < tup[1].evaluate():
                    judgement.append(Bool(True))
                else:
                ...
                raise ValueError(f'Invalid operator {self.op}')
        return BoolOp('and', judgement).evaluate()
